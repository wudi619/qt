    ...
    <div class="content">
      ...
      <div class="content_right">
          ...
          <div class="left firstItem">
            <van-popover v-model:show="showPopover" :show-arrow="false">
              <div class="rightScondList">
                <!-- 市价委托 -->
                <div class="rightScondListItem" :class="transactionLabel ? 'hightBlue' : ''" @click="onSelect(1)">
                  {{ _t18(`bb_market_order`) }}
                </div>
                <!-- 限价委托 -->
                <div class="rightScondListItem" :class="!transactionLabel ? 'hightBlue' : ''" @click="onSelect(0)">
                  {{ _t18(`bb_Limit_order`) }}
                </div>
              </div>
              <template #reference>
            ...
            <van-popover v-model:show="showPopoverNum" :show-arrow="false">
              <div class="rightScondListNum" v-if="numList.length > 0">
                <div v-for="item in numList" :key="item.id" class="rightScondListItem"
                  :class="item.name === transactionNum ? 'hightBlue' : ''" @click="selectNum(item)">
                  {{ item.name }}
                </div>
              </div>
              <template #reference>
        ...
        <div class="rightFifth" v-if="transactionLabel">
          <input type="number" v-model.trim="delegateTotal" class="amount"
            :placeholder="`${_t18(`metastaking_bb_count`)}(${_t18(`lots`)})`" />
        </div>
        <!-- 限价委托：价格，数量 -->
        <div class="rightFifth" v-if="!transactionLabel">
          <!-- 价格 -->
          <input type="number" class="amount" v-model.trim="delegatePrice" :placeholder="_t18(`price`)" />
        </div>
        <div class="rightFifth" v-if="!transactionLabel">
          <!-- 数量(手lots/张lots2) -->
          <input type="number" v-model.trim="delegateTotal" class="amount"
        ...
        <div class="rightFourth">
          <div class="lineBg">
            <div :class="sliderValue === 0 ? 'node' : index * 25 > sliderValue ? 'node' : 'node active'
              " v-for="(item, index) in 5" :key="index"></div>
          </div>
        ...
        <!-- 买 -->
        <div class="informationSheet" v-if="type==1">
          <!-- 可开空 paxpay特殊-->
          <div class="nineSix">
        ...
        <!-- 卖 -->
        <div class="informationSheet" v-if="type==2">
          <!-- 可开空 paxpay特殊-->
          <div class="nineSix">
            <p v-if="['paxpay'].includes(_getConfig('_APP_ENV'))">
...
import Handicap from '@/views/trade/components/common/handicap.vue'
import OrderListBox from '@/views/trade/components/ustandard/content/OrderList.vue'
...
console.log(props.coinInfo.coin, 288);
