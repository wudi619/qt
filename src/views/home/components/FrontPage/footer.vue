<template>
  <div class="footer">
    ...
    <van-nav-bar left-arrow>
      <template #left>
        <!-- <span class="title">{{ _t18('hang-qing') }}</span> -->
        <image-load style="height: 1rem;
    width: 1rem;" filePath="../../resource/images/light/logo9.png" alt="" class="carouselItem"  />
    <span class="title">BigOne</span>
      </template>
      <template #right>
        <van-icon class="title" name="search" size="18" @click="goToSearchPage" />
      </template>
    </van-nav-bar>
    ...
    <div class="headerChoose">
      <van-tabs v-model:active="showTab" line-width="20" line-height="2" title-active-color="#17AC74" color="#17ac74"
        title-inactive-color="#5d626d" @click-tab="handelShowTab(showTab)" shrink>
    ...
    <div class="main">
      ...
      <div v-if="showTab === 0 && headerList[0].show">
        ...
        <div v-if="currentCoinList0.length">
          <CurrencyItem v-for="(item, index) in currentCoinList0" :key="index" :currencyItem="item"
            @click="linkTo(item)">
          </CurrencyItem>
        </div>
        <Nodata v-else></Nodata>
      ...
      <div v-if="showTab === 1 && headerList[1].show">
        <div v-if="currentCoinList1.length">
          <CurrencyItem v-for="(item, index) in currentCoinList1" :key="index" :currencyItem="item"
            @click="linkTo(item)">
          </CurrencyItem>
        </div>
        <Nodata v-else></Nodata>
      ...
      <div v-if="showTab === 2 && headerList[2].show">
        <div v-if="currentCoinList2">
          <CurrencyItem v-for="(item, index) in currentCoinList2" :key="index" :currencyItem="item"
            @click="linkTo(item)">
          </CurrencyItem>
        </div>
        <Nodata v-else></Nodata>
...
import Header from '@/views/home/components/FrontPage/header.vue'
import { DIFF_HOME_BANNER } from '@/config/index'
import HeaderDiff from '@/views/home/components/FrontPage/headerDiff.vue'
import Menu from '@/views/home/components/FrontPage/menu.vue'
